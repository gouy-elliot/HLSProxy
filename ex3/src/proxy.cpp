...
Proxy::Proxy(const Url &cdn_url) : _cdn_url(cdn_url), _server_fd(-1), _manifest_parser(_cdn_url) {
        ...
        if (loop) {
            ...
            for (it = _clients.begin(); it != _clients.end(); ++it) {
                ...
                if (FD_ISSET((*it)->get_fd(), &read_set)) {
                    int size = ::read((*it)->get_fd() , (*it)->get_buffer(), PROXY_BUFFER_SIZE);
                    if (size > 0) {
                ...
                if (FD_ISSET((*it)->get_cdn_fd(), &read_set)) {
                    int size = ::read((*it)->get_cdn_fd() , (*it)->get_buffer(), PROXY_READ_SIZE);
                    if (size > 0) {
                        (*it)->print_out_request();
                        size = _manifest_parser.parse_manifest(*it, (*it)->get_buffer(), size);
